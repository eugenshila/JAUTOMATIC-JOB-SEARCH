...
6. **Merge the PR**, then publish the GitHub Release from that verified MSI:

   ```bash
   gh workflow run publish-release.yml -f run_id=<windows-installer run id>
   ```

   Empty `run_id` uses the latest successful `windows-installer` run on `main`.
   The workflow downloads the MSI (this has to happen on a GitHub runner — Azure
   blob storage is not reachable from every authoring environment), writes notes
   from `CHANGELOG.md` plus a SHA-256 via `tools/release_notes.py`, and creates
   `v<version>` with the installer attached.
