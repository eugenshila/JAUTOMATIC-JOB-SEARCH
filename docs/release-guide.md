...
Artifacts Uploaded to GitHub (dist/JAUTOMATIC-Setup-<version>-x64.msi)
               │
               ▼
Merge, then publish-release.yml attaches that MSI to a GitHub Release (e.g. v1.10.0)
```
...
* `jautomatic/__init__.py`: `__version__ = "1.10.0"`
...
6. **Merge the PR**. A green `windows-installer` run on `main` then triggers
   `publish-release.yml`, which downloads that MSI (this has to happen on a
   GitHub runner — Azure blob storage is not reachable from every authoring
   environment), writes notes from `CHANGELOG.md` plus a SHA-256 via
   `tools/release_notes.py`, and creates `v<version>` with the installer
   attached. If that tag already exists the publisher is a no-op, so later
   commits at the same version do not duplicate the release.

   To attach a specific earlier installer run instead:

   ```bash
   gh workflow run publish-release.yml -f run_id=<windows-installer run id>
   ```
