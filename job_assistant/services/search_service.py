...
class SearchOrchestrator:
    ...
    def run(self, config: dict[str, Any]) -> SearchSummary:
        ...
        feed_urls = [line.strip() for line in config.get("feed_urls", []) if line.strip()]
        if not feed_urls:
            self.repository.complete_search_history(search_id, status="no_sources")
            summary.errors.append("No public feed URLs are configured. Add an approved RSS/XML or JSON feed first.")
            return summary
        ...
        try:
            for feed_url in feed_urls:
                try:
                    records = self.connector.search(feed_url=feed_url, titles=titles, locations=locations)
